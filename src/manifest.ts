...
let manifestValidator: ValidateFunction<AdversaryManifest> | undefined;
...
export function validateAdversaryManifest(value: unknown): AdversaryManifest {
  ...
  validateDetectionSemantics(value.detection);
  return value;
}

function getManifestValidator(): ValidateFunction<AdversaryManifest> {
  ...
  manifestValidator = new Ajv2020({ allErrors: true, strict: true }).compile<AdversaryManifest>(
    schema,
  );
...
function validateDetectionSemantics(detection: DetectionManifest | undefined): void {
  if (detection?.entrypoint === undefined) return;
  ...
  if (invalid) {
    const issue = "manifest.detection.entrypoint must be a portable project-relative path";
    throw new ManifestValidationError(`invalid adversary manifest: ${issue}`, [issue]);
  }
}
